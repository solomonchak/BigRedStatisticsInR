...
# To add this to an existing Google Doc, first upload the whole docx to google, then copy and paste the relavent section
...
![](Images/NewProject.png){#fig-NewProject width="452"}
...
![](Images/upload.png){#fig-upload width="462"}
...
![](Images/Export.png){#fig-export width="451"}
...
![](Images/install_package.png){width="322"}
...
![](Images/filepanel.png){#fig-FilePanel}
...
| `data_name` is a name of the **object** (in the form of a dataframe, see below) that we are assigning to the output of `read.csv(file = "name.csv")` via a `=` sign.
| 
...
![](Images/env_panel.png){#fig-env_panel}
...
The object is a *dataframe*, which stores a two-dimensional table that is like a spreadsheet with columns and rows. There are many other classes of R object, see [here](https://swcarpentry.github.io/r-novice-inflammation/13-supp-data-structures/) for other classes in R. We will only be using dataframes in this guide.
...
With dataframe, you can show the column titles with:
...
![](Images/seashell%20data%20in%20excel.png){#fig-seashellexcel width="139"}
...
| `$` is an "operator" to get a column from a dataframe's table.
...
![](Images/Mean%20shell%20length%20by%20species.png){#fig-meanshell}
...
According to [VI. Importing data into R], import `fertilizer.csv` from the shared drive ([here](https://drive.google.com/drive/folders/18p2BJvPVNJhLtYQ-HolEADr5Hwpi67JB?usp=sharing) or <https://tinyurl.com/5dpk43x7>) as a daraframe named `fertilizer`. Also, be sure you are [Loading packages].
...
| 
| `ggplot()` specifies a dataframe used for plotting with `data = heightXtrt_summary` and the x and y variables to use with `aes(x = Treatment, y = Plant_height_cm)`.
| 
| `geom_col()` adds the bar graph.
| The bar graph to display the values in the column `Plant_height_cm`, which are the means of `Plant_height_cm` grouped by `Treatment` in the object `heightXtrt_summary.` Then, `fill="lightblue"` and `color = "black"` controls the color of the box and the color of the outline, respectively.
| 
...
| 
| You will need at least the first two lines (connected by +) to build a ggplot. For example: `ggplot(data = heightXtrt_summary, aes(x = Treatment, y = Plant_height_cm)) +    geom_bar(stat="identity")`
...
![](Images/download_graph.png){width="542"}
...
Type `?geom_boxplot` in the Console Panel to show the help page for `geom_boxplot()`. The values displayed by the hinges and whiskers are explained in the section "Summary statistics". Note that SD and SE are not displayed in the boxplot.
...
# If you need to change the (column) name on the dataframe, please see the section "Editing data in .csv and dataframe"
```
...
According to [VI. Importing data into R], import `penicillin.csv` from the shared drive ([here](https://drive.google.com/drive/folders/18p2BJvPVNJhLtYQ-HolEADr5Hwpi67JB?usp=sharing) or <https://tinyurl.com/5dpk43x7>) as a daraframe named `penicillin`. Also, be sure you are [Loading packages].
...
You can also show the p value of each comparison with:
...
Your final figure should include the post hoc letters, similar to the figure below. You can export the graph to add the letters manually. Alternatively, you can do it in R. See Appendix 2. [Adding post hoc grouping into bar graph and boxplot].
...
# But we first need to extract the letters from hsd to build a dataframe to help us label the plot
...
According to [VI. Importing data into R], import `chirps.csv` from the shared drive ([here](https://drive.google.com/drive/folders/18p2BJvPVNJhLtYQ-HolEADr5Hwpi67JB?usp=sharing) or <https://tinyurl.com/5dpk43x7>) as a daraframe named `chirps`. Also, be sure you are [Loading packages].
...
![Chi-square](Images/chisq.png){#chisq}.
...
According to [VI. Importing data into R], import `crust_sp1.csv` from the shared drive ([here](https://drive.google.com/drive/folders/18p2BJvPVNJhLtYQ-HolEADr5Hwpi67JB?usp=sharing) or <https://tinyurl.com/5dpk43x7>) as a daraframe named `crust_sp1`. Also, be sure you are [Loading packages].
...
![](Images/chisq_table.png){#fig-chisq_table}
...
![](Images/Contingency_data.png){#fig-Contingency_data}
...
We will begin by creating the data for species 2. Alternatively, according to [VI. Importing data into R], import `crust_sp2.csv` from the shared drive ([here](https://drive.google.com/drive/folders/18p2BJvPVNJhLtYQ-HolEADr5Hwpi67JB?usp=sharing) or <https://tinyurl.com/5dpk43x7>) as a daraframe named `crust_sp2`.
...
# rbind() combines two dataframes by adding the second datafrome (crust_sp2) to the bottom on the first dataframe (crust_sp1).
...
2.  (Advanced) Modify the dataframe that was loaded into R.\
    \
...
# Note that indexing for dataframe goes from row to column in R
...
## Appendix 2. Advanced graphing {data-link="How to graph in R"}

### Overlaying data points in bar graph and boxplot

When visualizing data, it is sometime beneficial to show the raw data. We can overlap the data points on a bar graph. This example uses the raw data `fertilizer` and the dataframe `heightXtrt_summary` generated by `summarySE` from [A. T-test].
...
# It requires adding a new argument geom_point(), where the arguments specify getting the raw data from fertilizer and "jittering" the points to avoid overlapping points with the same values. 

```

We can do the same for a boxplot. When an outlier is present, it will be displayed as a point by `geom_boxplot()`. Therefore, the argument `outlier.colour = "grey"` is used to specify the color of the outlier point.
...
# There is no outlier point in this example!
```

### Adding post hoc grouping into bar graph and boxplot
...
## Appendix 3. Create dataframes in R {data-link="How to graph in R"}
